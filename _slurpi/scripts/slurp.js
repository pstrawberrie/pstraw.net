...
function buildTMDBFavoritesArray(category) {
  ...
  let currentPage = 1;
  const url = () =>
    `https://api.themoviedb.org/3/account/${TMDBaccountId}/favorite/${cat}?language=en-US&page=${currentPage}&sort_by=created_at.asc`;
...
function saveTMDBDetails(category, arr) {
  ...
  const url = (id) =>
    `https://api.themoviedb.org/3/${cat}/${id}?language=en-US&append_to_response=${append}`;

  return new Promise(async (resolve, reject) => {
    try {
      console.log(
        `---> saving ${cat} details started (${arr.length} total)...`,
      );
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (!dbItem) {
          ...
          if (cat === "movie") {
            ...
            data.rating =
              details?.release_dates?.results
                ...
                ?.release_dates?.filter((c) => c.certification !== "")[0]
          ...
          if (!imageExists) {
            await saveImage(
              `https://image.tmdb.org/t/p/w200/${dbItem.poster_path}`,
              imagePath,
            );
          }
        }
